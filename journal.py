...
def get_pro_insight(symbol, d):
    ...
    TASK: Sei realistisch und professionell. Analysiere das Risiko basierend auf der Marktstruktur.
    Schreibe 3 kalte, professionelle Sätze auf Deutsch. Keine Einleitung, fang direkt an.
    """
...
def send_embed(symbol, d):
    ...
    embed.add_field(name="⚔️ Supply (Verkäufer)", value=r_text, inline=True)
    
    embed.add_field(name="🧠 Sentinel Insight", value=f"*{d['insight']}*", inline=False)
    embed.set_footer(text=f"Sentinel Alpha | MTF Dashboard | Realism Logic")
    
    webhook.send(embed=embed, username=f"Sentinel Elite")
